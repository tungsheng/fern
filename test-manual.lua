-- Manual test script for nvim-cursor
-- Run with: nvim --cmd "luafile test-manual.lua"

print("=== nvim-cursor Manual Test Suite ===\n")

-- Test 1: Check if module can be loaded
print("1. Testing module loading...")
local ok, vim_cursor = pcall(require, "nvim-cursor")
if ok then
  print("  ✓ nvim-cursor module loaded")
else
  print("  ✗ Failed to load nvim-cursor: " .. tostring(vim_cursor))
  return
end

-- Test 2: Check config module
print("\n2. Testing config module...")
local ok_config, config = pcall(require, "nvim-cursor.config")
if ok_config then
  ...
  if opts then
    ...
    print("    Model: " .. opts.api.cursor.model)
    print("    Timeout: " .. opts.api.cursor.timeout)
    print("    Max retries: " .. opts.api.cursor.max_retries)
  else
...
local ok_logger, logger = pcall(require, "nvim-cursor.logger")
if ok_logger then
...
local ok_errors, errors = pcall(require, "nvim-cursor.api.errors")
if ok_errors then
...
local ok_progress, progress = pcall(require, "nvim-cursor.ui.progress")
if ok_progress then
...
local ok_history, history = pcall(require, "nvim-cursor.history")
if ok_history then
...
local ok_health, health = pcall(require, "nvim-cursor.health")
if ok_health then
  print("  ✓ Health module loaded")
  print("  ℹ Run :checkhealth nvim-cursor to see full diagnostics")
else
...
local ok_context, context = pcall(require, "nvim-cursor.context")
if ok_context then
...
print("\nNext steps:")
print("1. Set CURSOR_API_KEY if not already set")
print("2. Run :checkhealth nvim-cursor in Neovim")
print("3. Try the plugin with <leader>ae on selected code")
print("\nFor detailed diagnostics, run:")
print("  :checkhealth nvim-cursor")
