local M = {}

local openai_base = require("fern.api.openai_base")
local config = require("fern.config")

function M.cancel_current()
  openai_base.cancel_current("cursor")
end

function M.send_request(prompt, context, options, on_chunk, on_complete, on_error, retry_count)
  local opts = config.get().api.cursor

  if not opts.api_key then
    opts.api_key = vim.env.CURSOR_API_KEY
  end

  openai_base.send_request("cursor", opts, prompt, context, options, on_chunk, on_complete, on_error, retry_count)
end

return M
